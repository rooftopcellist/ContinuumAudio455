	...
	if (isset($_FILES['upload'])) {
		...
		if (in_array($_FILES['upload']['type'], $allowed)) {
			...
			if (move_uploaded_file ($_FILES['upload']['tmp_name'], "../Uploads/$folder/{$_FILES['upload']['name']}")) {
				...
				$sql = "INSERT into Ca_User_Images (emailAddr, imageName, imageType) VALUES (:email, :name, :type)";
...
<div class= "container">
	...
	<form enctype="multipart/form-data" action="Ca_Upload_Images.php" method="post">
		<input type="hidden" name="MAX_FILE_SIZE" value="2097152">
		<fieldset>
			<legend>Select a JPEG or PNG image of 2M or smaller to be uploaded:</legend>
			<label for="file">
			File:<input class="btn btn-primary" type="file" name="upload" id="file"></label>
			<label for = "submit">
			<input class= "btn btn-success" type="submit" name="submit" value="Submit" id="submit"></label>
		</fieldset>
	</form>
</div>
